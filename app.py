...
LANG_MAP = {
    "HR 🇭🇷": {
        ...
        "note_vaga": "⚖️ **Napomena:** Navedene cijene su po jedinici, a točan iznos bit će poznat nakon vaganja.",
...
st.markdown("""<style>
    ...
    .product-card { background: #f9f9f9; border-radius: 8px; padding: 10px; border: 1px solid #ddd; text-align: center; margin-bottom: 10px; }
...
# --- 5. NAVIGACIJA ---
izabrani_jezik = st.sidebar.selectbox("Language", list(LANG_MAP.keys()))
...
if choice == T["nav_shop"]:
    ...
    col_proizvodi, col_desno = st.columns([0.6, 0.4])
    ...
    with col_desno:
        ...
        for p in proizvodi:
            ...
            if kol > 0:
                ...
                ukupno += kol * p['price']
                detalji += f"{p['name']} x {kol}\n"
        
        if ukupno > 0:
            st.write(f"### {T['total']}: {ukupno:.2f} €")
