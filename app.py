...
for idx, (proizvod, cijena) in enumerate(PROIZVODI.items()):
    with cols[idx % 3]:
        ...
        # Red s dva gumba: Dodaj i Smanji
        c1, c2 = st.columns(2)
        ...
        with c2:
            if st.button(f"➖ Smanji", key=f"rem_{proizvod}"):
                trenutna = st.session_state.cart.get(proizvod, 0)
                if trenutna > 1.0:
                    st.session_state.cart[proizvod] = trenutna - 0.5
                elif trenutna == 1.0:
                    del st.session_state.cart[proizvod]
                st.rerun()

        # Prikaz trenutnog stanja
        if proizvod in st.session_state.cart:
            st.info(f"U košarici: **{st.session_state.cart[proizvod]} kg**")
        ...
        if posalji:
            if not (ime_prezime and adresa and grad and telefon):
                ...
                try:
                    ...
                    st.balloons()
                    st.success("✅ Hvala Vam na narudžbi!")
                    st.session_state.cart = {}
