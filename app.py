...
LANG_MAP = {
    ...
    "DE 🇩🇪": {
        ...
        "note_vaga": "⚖️ **Hinweis:** Endpreis nach dem Wiegen.",
        ...
        "p4": "Slawonische Wurst", "p5": "Hausgematche Salami", "p6": "Geräucherte Knochen",
    ...
    with col_proizvodi:
        ...
        for i, p in enumerate(proizvodi):
            ...
            with sa_kolonom:
                ...
                if c1.button("➖", key=f"minus_{p['id']}"):
                    if st.session_state.cart.get(p['id'], 0) > 0: st.session_state.cart[p['id']] -= 1
                c2.markdown(f'<p class="qty-display">{st.session_state.cart.get(p["id"], 0)}</p>', unsafe_allow_html=True)
                if c3.button("➕", key=f"plus_{p['id']}"):
    ...
    with col_desno:
        ...
        if ukupno_eur > 0:
            ...
            with st.form("narudzba"):
                ...
                grad = st.text_input(T["form_city"]); adr = st.text_input(T["form_addr"])
                ptt = st.text_input(T["form_zip"]); drv = st.text_input(T["form_country"])
